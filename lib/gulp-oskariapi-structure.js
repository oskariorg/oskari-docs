...
function flush (callback) {
    ...
    var bundles = new Vinyl({
        path: 'bundles.json',
        contents: Buffer.from(JSON.stringify(getBundlesJson(), null, 2))
    });
    this.push(bundles);
    this.emit('data', bundles);

    var requests = new Vinyl({
        path: 'requests.json',
        contents: Buffer.from(JSON.stringify(getRequestsJson(), null, 2))
    });
    this.push(requests);
    this.emit('data', requests);

    var events = new Vinyl({
        path: 'events.json',
        contents: Buffer.from(JSON.stringify(getEventsJson(), null, 2))
    });
    this.push(events);
