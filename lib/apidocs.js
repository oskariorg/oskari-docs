...
var generatedDocsPath = __dirname + "/../md/generated/api";
...
function getChangelog(version, callback) {
    ...
    fs.readFile(mdDocPath, 'utf8', function (err, fileContent) {
        ...
        //var formatted = mdformatter.log(fileContent);
...
function getBundleDoc(version, bundle, callback) {
    ...
    var mdDocPath = path.join(generatedDocsPath, version, bundle, 'bundle.html');
    fs.readFile(mdDocPath, 'utf8', function (err, fileContent) {
        if (err) {
        ...
        //var formatted = mdformatter.bundle(fileContent);
        callback(fileContent);
    });
}
